...
class Customer {
  ...
  constructor(ftName: string, lName: string, age: number, gender: string, mob: number, acc: number) {
...
if (serv.service === "View Balance") {
  ...
  if (!account) {
    ...
    console.log(
      chalk.cyan.bold(
        `Dear ${name?.firstName} ${name?.lastName} your account balance is: $${account.balance}`
      )
    );
  }
}
